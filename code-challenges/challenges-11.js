...
function gradesAvg(arr) {
    let resultArr = arr.map((element) => {
        let sum = 0;
        let summation = element.gradsList.map((element1) =>
            sum += element1
        )
        let avg = summation[element.gradsList.length - 1] / element.gradsList.length
        console.log(avg)
        element.avg = avg
        return (
            element
