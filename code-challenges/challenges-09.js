...
const employeesBonus = (arr) => {
    arr.forEach(element => {
        let salaryEl = element.salary;
        let salarySliced = salaryEl.slice(0, 4);
        let salaryInt = parseInt(salarySliced);
        if (element.workHours > 8) {
            salaryInt += 100;
        } else {
            salaryInt += 50;
        }
        let salaryString = salaryInt.toString();
        element.salary = salaryString + '$';
    });
    return arr;
}
// -------------------------------------------------------------------------------------------------------
...
const mostExpensive = (budget, mouseArray, keyBoardArray) => {
    let bestCombo = mouseArray[0] + keyBoardArray[0];
    for (let i = 0; i < mouseArray.length; i++) {
        for (let j = 0; j < keyBoardArray.length; j++) {
            if (mouseArray[i] + keyBoardArray[j] <= budget && mouseArray[i] + keyBoardArray[j] > bestCombo) {
                bestCombo = mouseArray[i] + keyBoardArray[j];
            }
        }
    }
    return bestCombo;
