...
const objLat = (obj) => {
    ...
    let firstChar = fName.slice(0, 1).toUpperCase() + fName.slice(1);
    let lName = obj.lastName;
    let firstChar2 = lName.slice(0, 1).toUpperCase() + lName.slice(1);
    let output = `my name is ${firstChar} ${firstChar2} I am ${obj.age} YO, and I love ${obj.hobby}.`
    ...
    for (let i = 0; i < data.grades.length; i++) {
        for (let j = 0; j < data.grades[i].classes.length; j++) {
            for (let z = 0; z < data.grades[i].classes[j].classScores.length; z++) {
                ...
                average = Math.floor(sum / (data.grades[i].classes[j].classScores.length));
            }
            sum = 0;
            data.grades[i].classes[j].avg += average;
        }
    }
    return data;
};
