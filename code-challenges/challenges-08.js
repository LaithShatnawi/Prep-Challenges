...
const customerAndAge = (obj) => {
  let customerInfo = [];
  for (const customerName in obj) {
    customerInfo.push(`Customer Name :${customerName} , Age :${obj[customerName]}`);
  }
  return customerInfo;
};
// -------------------------------------------------------------------------------------------------------
...
const getStudents = (arr) => {
  ...
  for (let i = 0; i < courses.length; i++) {
    for (let j = 0; j < courses[i].Students.length; j++){
        if(courses[i].Students[j] === arr[z]){
          studentsInfo.push({'Student':arr[z],'course':courses[i].course});
        }
      }
    }
  }
